...
df_sample = pd.read_csv("../input/predict-location01/submission.csv")
...
for site_id in tqdm(list_site):
    ...
    df_train_pred = pd.read_csv(f"../input/predict-location01/train/{site_id}.csv")
    df_test_pred = pd.read_csv(f"../input/predict-location01/test/{site_id}.csv")
    ...
    wificols = sum([[f"strong_wifi{i}_x",f"strong_wifi{i}_y",f"strong_wifi_val{i}"] for i in range(3)],[])
    ibeaconcols = sum([[f"strong_ibeacon{i}_x",f"strong_ibeacon{i}_y",f"strong_ibeacon_val{i}"] for i in range(2)],[])

    # make train mart
    for shift_len in range(1,10):
        df_train_pred[f"waypoint_x_pred_prev{shift_len}"] = df_train_pred.groupby("path_id").shift(shift_len)["x"]
        df_train_pred[f"waypoint_y_pred_prev{shift_len}"] = df_train_pred.groupby("path_id").shift(shift_len)["y"]
        df_train_pred[f"waypoint_x_pred_next{shift_len}"] = df_train_pred.groupby("path_id").shift(-shift_len)["x"]
        df_train_pred[f"waypoint_y_pred_next{shift_len}"] = df_train_pred.groupby("path_id").shift(-shift_len)["y"]
    df_train = pd.merge(df_train_pred,df_train_41[["path_id","timestamp"]+wificols],how="left",on=["path_id","timestamp"])
    ...
    # make test mart
    for shift_len in range(1,10):
        df_test_pred[f"waypoint_x_pred_prev{shift_len}"] = df_test_pred.groupby("path_id").shift(shift_len)["x"]
        df_test_pred[f"waypoint_y_pred_prev{shift_len}"] = df_test_pred.groupby("path_id").shift(shift_len)["y"]
        df_test_pred[f"waypoint_x_pred_next{shift_len}"] = df_test_pred.groupby("path_id").shift(-shift_len)["x"]
        df_test_pred[f"waypoint_y_pred_next{shift_len}"] = df_test_pred.groupby("path_id").shift(-shift_len)["y"]        
    df_test_pred = df_test_pred
    df_test = pd.merge(df_test_pred,df_test_41[["path_id","timestamp"]+wificols],how="left",on=["path_id","timestamp"])
    ...
    X_train = df_train.drop(["path_id","timestamp","waypoint_x","waypoint_y","x_diff","y_diff","floor"], axis = 1)
    X_test = df_test.drop(["site_path_timestamp","site_id","path_id","timestamp","floor"], axis = 1)
